...
typedef enum {
    CONNECT_TYPE_UPLOAD = 0,
    CONNECT_TYPE_DOWNLOAD = 1,
...
static void server_controller_task(void *pvParameters){
    ...
    while (1){
        ...
        case CONNECT_TYPE_UPLOAD:
            ...
            start_upload(main_sock, service_packet.file_name, service_packet.file_size);
            break;
        case CONNECT_TYPE_DOWNLOAD:
            show_status_load(STATUS_DOWNLOAD);
            show_file_name(service_packet.file_name);
            start_download(main_sock, service_packet.file_name);
            break;
        default:
